...
class InitializeButton(Button):
    ...
    def clickAction(self):
        ...
        UI["field"].network = []
        UI["field"].network.append([{"weights":[random(0.01, 1) for i in range(UI["field"].nInputs + 1)], "EE": 0.01, "lastDelta": 0.001} for i in range(UI["field"].nPerLayer)])
        cambio = (UI["field"].nPerLayer - numberOfClasses) // UI["field"].nHidden
        curNPerLayer = UI["field"].nPerLayer - cambio
        curNPerLayer = max(curNPerLayer, numberOfClasses)
        lastNPerLayer = UI["field"].nPerLayer
        for nn in range(UI["field"].nHidden - 1):
            UI["field"].network.append([{"weights":[random(0.01, 1) for i in range(lastNPerLayer + 1)], "EE": 0.01, "lastDelta": 0.001} for i in range(curNPerLayer)])
            lastNPerLayer = curNPerLayer
            curNPerLayer -= cambio
            curNPerLayer = max(curNPerLayer, numberOfClasses)
        UI["field"].network.append([{"weights":[random(0.01, 1) for i in range(lastNPerLayer + 1)], "EE": 0.01, "lastDelta": 0.001} for i in range(UI["field"].nOutputs)])
...
class Field(Obj):
    def __init__(self, x, y, w, n):
        ...
        self.differentShape = [[0 for i in range(self.n)] for j in range(self.n)] # n * m matrix
        self.n = n - 1
    ...
    def display(self):
        ...
        for i in range(self.n + 1):
            for j in range(self.n + 1):
                ...
                if self.differentShape[i][j] == 1: 
                    triangle(self.x + i * self.space, self.y + j * self.space - self.pointSz, 
                             self.x + i * self.space + self.pointSz, self.y + j * self.space + self.pointSz, 
                             self.x + i * self.space - self.pointSz, self.y + j * self.space - self.pointSz)
                    # triangle(self.x + i * self.space, self.y + j * self.space - self.pointSz, 
                    #          self.x + i * self.space + self.pointSz, self.y + j * self.space + self.pointSz, 
                    #          self.x + i * self.space - self.pointSz, self.y + j * self.space + self.pointSz)
                else: circle(self.x + i * self.space, self.x + j * self.space, self.pointSz)
        if self.train:
            self.nextEpoch()
        # if self.showLines:
        #     # print("here")
    ...
    def backPropagation(self, expected):
        for i in reversed(range(len(self.network))):
            ...
            if i != len(self.network) - 1:
                ...
                for j in range(len(layer)):
                    perceptron = layer[j]
                    errors.append(-2.0 * (expected[j] - perceptron["output"]))
            for j in range(len(layer)):
                perceptron = layer[j]
                perceptron["delta"] = errors[j] * self.transferDerivative(perceptron["n"])
        
    # def quickProp(self, perceptron, curE):
    #     actE = curE * perceptron["output"]
    #     divisionEsZero = (perceptron["EE"] - actE)
    #     if divisionEsZero == 0: divisionEsZero = 0.0001
    #     temp = (actE / (divisionEsZero)) * perceptron["lastDelta"]
    #     if temp > self.learningRate * perceptron["lastDelta"]:
    #         temp = self.learningRate * perceptron["lastDelta"]
    #     foo = 0.0
    #     if perceptron["EE"] * actE < 0.0:
    #         foo = temp
    #     else:
    #         foo = temp + (1.75) * actE
    ...
    def clipx(self, val, mn, mx):
        val = max(val, mn)
        val = min(val, mx)
        return val
    
    def clipMxGrowth(self, dw, dwPrev, maxGrowth = 1.75):
        mxG = maxGrowth * abs(dwPrev)
        return self.clipx(dw, -mxG, mxG)
 
    def quickProp(self, perceptron, curE):
        gdRate = 0.001
        qp = 1.5
        lastGradiente = perceptron["EE"]
        gradiente = perceptron["delta"] * perceptron["output"]
        gradientStep = -gdRate * gradiente
        divFactor = lastGradiente - gradiente
        deltaW = 0
        #Deciding to do quickProp step or gradientDescent step 
        if(abs(divFactor) < 1e-9):
            deltaW = gradientStep
        else:
            deltaW = perceptron["lastDelta"] / divFactor
        deltaW = deltaW * gradiente
        #Checking if the deltas go in the same sign
        deciderSlope = abs(self.sign(deltaW) + self.sign(perceptron["lastDelta"])) 
        deciderSlope = self.clipx(deciderSlope, 0, 1)
        print(lastGradiente, gradiente, gradientStep, deltaW)
        #Gradient step to avoid deltaW vanishingly samll
        if deciderSlope == 1:
            deltaW = deltaW + (-qp * gradiente)
        deltaW = self.clipMxGrowth(deltaW, perceptron["lastDelta"])
        print(deltaW) 
        perceptron["EE"] = gradiente
        perceptron["lastDelta"] = deltaW
    ...
    def updateWeights(self, row):
        for i in range(len(self.network)):
            ...
            for perceptron in self.network[i]:
                self.quickProp(perceptron, perceptron["delta"])
                for j in range(len(inputs)):
                    perceptron["weights"][j] -= self.learningRate * perceptron["lastDelta"] * inputs[j]
                perceptron["weights"][-1] -= self.learningRate * perceptron["lastDelta"]
